desstat <- function(x,group){
  ...
  c(
    ...
    # within person SD
    wSD = round(mean(unlist(aggregate(x, by=list(group), FUN=sd, na.rm = TRUE)[2]),na.rm=TRUE),3),

    # between person SD
    bSD = round(sd(unlist(aggregate(x, by=list(group), FUN=mean, na.rm = TRUE)[2]),na.rm=TRUE),3),

    # ICC: how much of the variance is between-person
    ICC = round(multilevel.icc(x, cluster = group),3),
...
summarydesstat <- function(df){
  ...
  c("df$withinRSD",round(mean(df[df$firstbeep,"person_withinRSD"]),3), 0, round(sd(df[df$firstbeep,"person_withinRSD"]),3), 1,nrow(df[df$firstbeep,])),
